...
public class AdvancedStats extends Activity implements
    ...
    private void calcAndDispStats() {
        ...
        if (curSelection.equals("Best lineups")) {
            ...
            calcStatFromMap(mGamesAdded, new int[][]{{0,1,2,3,4,5}});
        }
    }

    public static List<String> sortByPosition(List<String> playerIds) {
            ...
            for(int k = 0; k < games.size(); k++) {
                ...
                for (MetaStatDb action : metaStats) {
                    ...
                    if (!action.getStatType().equals(DatabaseHelper.COL_GOALS) &&
                            !action.getStatType().equals("away_goal")) {
                        continue;
                    }
                    if (lineUp == null ||lineUp.size() == 0 ) {
                        Log.d("AdvancedStats", "Didn't find lineup");
                        lineUp = timeArray.get(action.getTimeOfAction() - 1);
                    }
    ...
    public static int getPositionFromArrPosition(int i) {
        if (i == 0 || i == 1 || i == 2) {
            return 0;
        } else if (i == 3) {
            return 1;
